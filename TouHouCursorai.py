...
ENEMY_COLOR = "green"
ENEMY_COLOR_HIT = "cyan"
PLAYER_COLOR = "red"
PLAYER_COLOR_HIT = "yellow"
...
class Enemy:
    def __init__(self, canvas, x, y, cooldown=5):
        ...
        self.hp = 2
        self.speed = 2
        self.shape = canvas.create_rectangle(x, y, x + 10, y + 10, fill=ENEMY_COLOR)
...
class Bullet:
    def __init__(self, canvas, x, y, dx, dy, from_):
        ...
        self.dy = dy
        if from_ == 0:
            self.shape = canvas.create_rectangle(x + 3, y, x + 7, y + 5, fill=PLAYER_BULLET_COLOR)
        else:
...
class GameScene:
    def __init__(self, master):
        ...
        self.score_text = self.canvas.create_text(5, HEIGHT - 20, text=f"Score: {self.player.score:09d}", anchor="w", fill="white")
        self.hp_text = self.canvas.create_text(5, HEIGHT - 5, text="HP: " + self.player.hp * "♥ ", anchor="w", fill="red")
    ...
    def update(self):
        if not self.pause:
            ...
            self.canvas.itemconfig(self.hp_text, text="HP: " + self.player.hp * "♥ ",)
